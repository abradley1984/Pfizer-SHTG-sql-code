...
with pat_list as
    ...
    /*  select avg(BMI), BMI_category, count(patid) from BMI_category
     ...
     combined as (select patid,
                         ...
                         enhanced_risk,
                         case
                             when diabetes = 1 and nash = 1 then 'N and D'
                             ...
                             when (nash = 0 or nash is null) and (diabetes = 0 or diabetes is null) and obesity = 1
                                 then 'Obesity only'
                             else 'None'
                             end
                             as t10_category
...
select count(patid), t10_category, BMI_category, v_high_risk, enhanced_risk, ascvd
from combined
group by t10_category, v_high_risk, enhanced_risk, ascvd, BMI_category
union
select count(patid), t10_category, BMI_category,'total', 'total',2
from combined
group by t10_category, BMI_category
;