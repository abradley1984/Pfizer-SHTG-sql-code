/*For Q2 T7 and T8
  ...
  Each site will need to change their database reference, currently set to:
  cdm.dbo. for cdm
  foo.dbo. for tables written
...
from (
    select distinct a.patid, cohort, 1 as High_Intensity_Statin, rx_frequency, RX_DOSE_ORDERED, rx_prn_flag, rx_days_supply, raw_rx_med_name, case
    when rxnorm_cui in ('859751', '859753', '2167565', '2167567', '2535745', '2536062') then 'dose_20'
    when rxnorm_cui in ('259255', '262095', '404011', '404013', '597984', '597990', '597993', '617311', '617320', '750207', '750211', '750215', '750235', '750239', '859419', '859421', '876514', '1422096', '1422098', '1422099', '1422101', '2167569', '2167571', '2535749', '2536064') then 'dose_over_40' end as level1
    from #pat_list a
    left join cdm.dbo.prescribing b on a.patid=b.patid
    where rx_order_Date BETWEEN '2020-09-30' AND '2021-09-30'
...
from (
    ...
    ('2447',
                                      ...
                                      '1048452',
                                      '1801279',
                                      '1801280') group by a.patid, cohort) c;
...
from (
    select distinct a.patid, a.cohort, ldl_result_num, nhdl, TG_result_num,
    -- LDL_category,
    LDL_category2, TG_category,
    --nhdl_category2,
    nhdl_category2, Statin, High_Intensity_Statin, Ezetimibe, bile_acid_sequestrant, fibrate, pcsk9, omega_3, niacin, icosapent_ethyl, first_therapy_date, last_therapy_date_in_sp
    from #pat_list a
...
from (
    select distinct patid, cohort, coalesce (Ezetimibe, bile_acid_sequestrant, fibrate, pcsk9, icosapent_ethyl, niacin, omega_3, 0) as other_lipid_lowering, case
    when (coalesce (statin, Ezetimibe, bile_acid_sequestrant, fibrate, pcsk9, icosapent_ethyl, niacin, omega_3, 0)) = 0
    then 1 end as no_lipid_lowering, case
    when (coalesce(statin, Ezetimibe, bile_acid_sequestrant, fibrate, pcsk9, icosapent_ethyl, niacin, 0)) = 0
    then 1 end as no_lipid_lowering_ex_omega, coalesce(Ezetimibe, bile_acid_sequestrant, fibrate, pcsk9, icosapent_ethyl, niacin, 0) as other_lipid_lowering_ex_omega, Statin, High_Intensity_Statin, Ezetimibe, bile_acid_sequestrant, fibrate, pcsk9, omega_3, niacin, icosapent_ethyl, first_therapy_date, last_therapy_date_in_sp, IIF(LDL_result_num >= 100, 1, 0) as ldl_above_100, IIF(LDL_result_num >= 70, 1, 0) as ldl_above_70, IIF((LDL_result_num < 70 and nhdl >= 100), 1, 0) as ldl_under_70_nhdl_above_100, IIF((LDL_result_num < 70 and nhdl >= 100 and TG_result_num >= 150), 1, 0) as ldl_under_70_nhdl_above_100_TG_above_150, IIF(nhdl >= 100, 1, 0) as nhdl_above_100, IIF(nhdl >= 130, 1, 0) as nhdl_above_130, IIF(TG_result_num >= 150, 1, 0) as TG_above_150, IIF(datediff(dd, first_therapy_date, last_therapy_date_in_sp) < 90, 1, 0) as less_than_3_months_therapy
    FROM #all_meds) n;
