...
select * into #labs_all
from (select * from Q2_labs_all) as [Q2la*];
--Risk status
--Recent ACS (12 months)
select * into #recent_ACS
from (select pats.patid, 1 as recent_ACS
    from #pat_list pats
...
from (select lab_result_cm.patid,
 ...
 lab_result_cm.result_date<'2021-09-30' and
        ...
        AND not lab_result_cm.result_unit in
                ('mg/d', 'g/dL', 'mL/min/{1.73_m2}', 'mL/min') --Excluding rare weird units   --AND lab_result_cm.result_num < 1000
