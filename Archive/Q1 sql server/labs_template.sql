...
DECLARE @DATE_END DATE = '2022-05-31'; drop table if exists #pat_list
select STUDY_ID
into #pat_list
FROM [VaxHez].live.DEMOGRAPHIC_PITT;

--defining new labs table
...
from (select a.patid,
      ...
      FROM #pat_list a
               left join #labs on a.patid = patid
      WHERE
      --removing references to dates
      --result_date BETWEEN '2020-09-30' AND '2021-09-30'

       -- AND
        lab_loinc in ('46986-6', '13458-5', '2091-7')
...
from (select a.patid,
      ...
      from #pat_list a
               left join #labs on a.patid = patid
      WHERE
      --take out date references
      --result_date BETWEEN '2020-09-30' AND '2021-09-30'

       -- AND
        lab_loinc in ('1884-6', '1871-3', '1881-2')
