...
select * from (
select count(distinct patid) as N ,'Have lab data' as label1 ,2 as order1 from foo.dbo.SHTG_Q2_STEP1_pre_exc
    union
select count(distinct patid) ,'Have lab data and over 18',3 from foo.dbo.SHTG_Q2_STEP1_pre_exc
where age>=18
union
select count(distinct patid) ,'Have lab data, over 18 and at least 180 days since first encounter',4 from foo.dbo.SHTG_Q2_STEP1_pre_exc
