...
where p.provider_specialty_primary in
      ('208D00000X', '163WG0000X', '207Q00000X', '207RC0000X', '207RA0001X', '207RC0001X', '207RI0011X', '2080P0202X',
       '163WE0003X', '207P00000X', '207PE0004X', '207PP0204X', '207PS0010X', '207PT0002X', '2080P0204X', '261QE0002X',
       '364SE0003X', '207RE0101X', '2080P0205X', '207RG0100X', '207QA0000X', '207QA0505X', '207R00000X', '207RA0000X',
...
from (select a.patid/*, provider_specialty as provider_specialty_a*/
      from #providers a
      where a.provider_specialty = 'endo') b
         inner join (select c.patid patid /*, provider_specialty as provider_specialty_b*/
                     from #providers c
...
group by cohort*/
         union
         select '2' as order1, 'age' as label1, 'STD age' as label2, round(stdev(age), 2) as N, cohort
         from #pat_list
         group by cohort
         union
...
--group by cohort
         ...
         select '3' as order1, 'sex' as label1, SEX_category as label2, count(distinct patid) as N, cohort
         from #sex_category
         group by cohort, SEX_category

         union
         select '4' as order1, 'race' as label1, RACE_category as label2, count(distinct patid) as N, cohort
         from #race_category
         group by cohort, race_category
         union
         select '5'                   as order1,
                'hispanic ethnicity'  as label1,
                hispanic_category     as label2,
                count(distinct patid) as N,
                cohort
         from #race_category
         group by cohort, hispanic_category
         union
         select '8' as order1, 'Insurance' as label1, insurance_type as label2, count(distinct patid), cohort
         from #insurance_type
         group by insurance_type, cohort

         union
         select '8' as order1, 'Insurance' as label1, 'has_insurance_info' as label2, count(distinct patid), cohort
         from #insurance

         group by cohort
         union
         select '6' as order1, 'Smoking' as label1, smoking_category as label2, count(distinct a.patid), a.cohort
         from #pat_list a
                  left join #smoking_category b on a.patid = b.patid
         group by a.cohort, smoking_category
         union
         select '9' as order1, 'pre-index_days' as label1, 'Mean' as label2, avg(PRE_INDEX_DAYS) as N, cohort
         from #pat_list
         group by cohort
/*union
...
group by cohort*/
         union
         select '9' as order1, 'pre-index_days' as label1, 'STD' as label2, stdev(PRE_INDEX_DAYS) as N, cohort
         from #pat_list
         group by cohort
         union
         select '9'              as order1,
                'pre-index_days' as label1,
                'pct_25'         as label2,
                PERCENTILE_CONT(0.25) WITHIN
...
--group by cohort
         ...
         select '7' as order1, 'Provider' as label1, provider_specialty as label2, count(distinct patid), cohort
         from #providers
